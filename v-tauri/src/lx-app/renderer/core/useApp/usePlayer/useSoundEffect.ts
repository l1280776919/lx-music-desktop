...
const filterModules = import.meta.glob('@renderer/assets/medias/filters/*', {
  eager: true,
  import: 'default',
}) as Record<string, string>
const getFilterPath = (name: string) => {
  const entry = Object.entries(filterModules).find(([key]) => key.endsWith('/' + name))
  if (!entry) throw new Error(`missing filter asset: ${name}`)
  return entry[1]
}
const loadBuffer = async(name: string) => new Promise<AudioBuffer>((resolve, reject) => {
  const path = getFilterPath(name)
  if (cache.has(path)) {
