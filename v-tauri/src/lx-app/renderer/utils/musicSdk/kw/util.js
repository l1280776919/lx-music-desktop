...
const createAesEncrypt = (buffer, mode, key) => {
  if (mode != 'aes-128-ecb') throw new Error(`Unsupported AES mode: ${mode}`)
...
const createAesDecrypt = (buffer, mode, key) => {
  if (mode != 'aes-128-ecb') throw new Error(`Unsupported AES mode: ${mode}`)
  const decrypted = CryptoJS.AES.decrypt({
    ciphertext: bufferToWordArray(buffer),
